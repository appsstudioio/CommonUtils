...
private extension Combine.Publishers.ControlTarget {
    private final class Subscription<S: Subscriber, CControl: AnyObject>: Combine.Subscription where S.Input == Void {
        private var subscriber: S?
        weak private var control: CControl?

        private let removeTargetAction: (CControl?, AnyObject, Selector) -> Void
        private let action = #selector(handleAction)

        init(subscriber: S,
             control: CControl,
             addTargetAction: @escaping (CControl, AnyObject, Selector) -> Void,
             removeTargetAction: @escaping (CControl?, AnyObject, Selector) -> Void) {
